...
// ================================================================
// CLASS: Garaje  (antes Garage) — dibujo de carros y UI del garaje
// ================================================================
class Garaje {
    static #viewer = null;
    ...
    static dibujarMini(canvas, tipo, color) {
        ...
        Garaje.#dibujarPerfil(ctx, tipo, color);
        ctx.restore();
    }

    static #dibujarPerfil(ctx, tipo, color) {
        switch (tipo) {
            case 'deportivo': Garaje.#perfilDeportivo(ctx, color); break;
            case 'suv':       Garaje.#perfilSUV(ctx, color);       break;
            case 'muscle':    Garaje.#perfilMuscle(ctx, color);    break;
            case 'formula':   Garaje.#perfilFormula(ctx, color);   break;
            case 'pickup':    Garaje.#perfilPickup(ctx, color);    break;
            case 'clasico':   Garaje.#perfilClasico(ctx, color);   break;
        }
    }
    ...
    static dibujar3D(canvas, tipo, color) {
        ...
        Garaje.#dibujar3DTipo(ctx, tipo, color);
        ctx.restore();
        ...
        ctx.fillText(Garaje.#TIPOS[tipo]?.desc || '', W/2, H - 8);
        ctx.shadowBlur = 0;
    }

    static #dibujar3DTipo(ctx, tipo, color) {
        switch (tipo) {
            case 'deportivo': Garaje.#carro3DDeportivo(ctx, color); break;
            case 'suv':       Garaje.#carro3DSUV(ctx, color);       break;
            case 'muscle':    Garaje.#carro3DMuscle(ctx, color);    break;
            case 'formula':   Garaje.#carro3DFormula(ctx, color);   break;
            case 'pickup':    Garaje.#carro3DPickup(ctx, color);    break;
            case 'clasico':   Garaje.#carro3DClasico(ctx, color);   break;
        }
    }
    ...
    static iniciar(estado, mostrar) {
        ...
        document.querySelectorAll('.carro-mini-canvas').forEach(c =>
            Garaje.dibujarMini(c, c.dataset.tipo, estado.color)
        );
        ...
        document.getElementById('btn-ver-carro').addEventListener('click', () => {
            ...
            document.getElementById('preview-nombre').textContent =
                Garaje.#TIPOS[tipoSel]?.nombre || tipoSel;

            const canvas = document.getElementById('canvas-preview-3d');
            Garaje.#viewer?.detener();
            Garaje.#viewer = new Visor3D(canvas);
            Garaje.#viewer.cargar(tipoSel, colorPreview);
            ...
            document.getElementById('colores-preview').onclick = e => {
                ...
                Garaje.#viewer?.cambiarColor(colorPreview);
                document.querySelectorAll('.carro-mini-canvas').forEach(c =>
                    Garaje.dibujarMini(c, c.dataset.tipo, colorPreview)
                );
            };
        });
    ...
    static #perfilDeportivo(ctx, color) {
        ...
        Garaje.#rueda(ctx,-28,4,10);
        Garaje.#rueda(ctx,26,4,10);
    }
    ...
    static #perfilSUV(ctx, color) {
        ...
        ctx.closePath(); ctx.fill();
        ctx.fillStyle = Garaje.#brillante(color,-30); ctx.fillRect(-44,-12,88,4);
        ctx.fillStyle = '#ffffaa';
        ctx.beginPath(); ctx.roundRect(42,-28,5,7,1); ctx.fill();
        ctx.fillStyle = '#ff3333';
        ctx.beginPath(); ctx.roundRect(-48,-28,4,10,1); ctx.fill();
        Garaje.#rueda(ctx,-28,4,11);
        Garaje.#rueda(ctx,26,4,11);
    }
    ...
    static #perfilMuscle(ctx, color) {
        ...
        ctx.closePath(); ctx.fill();
        ctx.fillStyle = Garaje.#brillante(color,25);
        ctx.beginPath();
        ...
        Garaje.#rueda(ctx,-30,5,12);
        Garaje.#rueda(ctx,28,5,12);
    }
    ...
    static #perfilFormula(ctx, color) {
        ...
        ctx.fillStyle = Garaje.#brillante(color,-20);
        ctx.fillRect(-54,-20,10,3); ctx.fillRect(-50,-20,3,8); ctx.fillRect(46,-14,10,3);
        Garaje.#rueda(ctx,-34,3,9);
        Garaje.#rueda(ctx,30,3,9);
        ctx.fillStyle = '#1a1a1a';
    ...
    static #perfilPickup(ctx, color) {
        ...
        ctx.closePath(); ctx.fill();
        ctx.strokeStyle = Garaje.#brillante(color,-50); ctx.lineWidth = 1.5;
    ...
    static #perfilClasico(ctx, color) {
        ...
        ctx.closePath(); ctx.fill();
        ctx.fillStyle = Garaje.#brillante(color,28);
        ctx.beginPath();
    ...
    static #carro3DDeportivo(ctx, color) {
        const cl = Garaje.#brillante(color,40), cd = Garaje.#brillante(color,-50), cs = Garaje.#brillante(color,-20);
        Garaje.#caraLateral(ctx,[[-90,0],[-90,-38],[-60,-55],[40,-55],[85,-38],[85,0]],color);
        Garaje.#caraLateral(ctx,[[-58,-55],[-30,-80],[30,-80],[60,-55]],cl);
        Garaje.#caraLateral(ctx,[[85,0],[95,-10],[95,-42],[85,-38]],cs);
        Garaje.#caraLateral(ctx,[[85,-38],[95,-42],[65,-58],[60,-55]],cd);
        ...
        Garaje.#rueda3D(ctx,-52,4,22,14);
        Garaje.#rueda3D(ctx,52,4,22,14);
    }

    static #carro3DSUV(ctx, color) {
        const cl = Garaje.#brillante(color,35), cd = Garaje.#brillante(color,-45), cs = Garaje.#brillante(color,-15);
        Garaje.#caraLateral(ctx,[[-88,0],[-88,-52],[-70,-70],[65,-70],[85,-52],[85,0]],color);
        Garaje.#caraLateral(ctx,[[-68,-70],[-68,-100],[62,-100],[62,-70]],cl);
        Garaje.#caraLateral(ctx,[[85,0],[96,-10],[96,-56],[85,-52]],cs);
        Garaje.#caraLateral(ctx,[[85,-52],[96,-56],[64,-72],[62,-70]],cd);
        ...
        Garaje.#rueda3D(ctx,-54,4,24,16);
        Garaje.#rueda3D(ctx,54,4,24,16);
    }

    static #carro3DMuscle(ctx, color) {
        const cl = Garaje.#brillante(color,40), cd = Garaje.#brillante(color,-45), cs = Garaje.#brillante(color,-18);
        Garaje.#caraLateral(ctx,[[-92,0],[-92,-42],[-65,-58],[55,-58],[90,-42],[90,0]],color);
        Garaje.#caraLateral(ctx,[[20,-58],[40,-70],[75,-50],[90,-42],[55,-58]],Garaje.#brillante(color,20));
        Garaje.#caraLateral(ctx,[[-63,-58],[-35,-82],[30,-82],[55,-58]],cl);
        Garaje.#caraLateral(ctx,[[90,0],[100,-10],[100,-46],[90,-42]],cs);
        Garaje.#caraLateral(ctx,[[90,-42],[100,-46],[76,-53],[75,-50]],cd);
        ctx.fillStyle='rgba(100,200,255,0.48)';
        ...
        Garaje.#rueda3D(ctx,-56,4,26,17);
        Garaje.#rueda3D(ctx,56,4,26,17);
    }

    static #carro3DFormula(ctx, color) {
        const cd = Garaje.#brillante(color,-40);
        Garaje.#caraLateral(ctx,[[-95,-5],[-95,-20],[-50,-28],[55,-28],[90,-18],[90,-5]],color);
        Garaje.#caraLateral(ctx,[[-95,-5],[-95,-20],[-50,-28],[55,-28],[90,-18],[90,-5],[95,-8],[95,-22],[56,-32],[-50,-32],[-100,-23],[-100,-8]],cd);
        ctx.fillStyle=Garaje.#brillante(color,30);
        ...
        Garaje.#caraLateral(ctx,[[-100,-24],[-100,-30],[-75,-30],[-75,-24]],cd);
        ctx.fillStyle=cd; ctx.beginPath(); ctx.roundRect(-92,-30,5,20,1); ctx.fill();
        Garaje.#caraLateral(ctx,[[80,-18],[96,-12],[96,-18],[80,-22]],cd);
        Garaje.#rueda3D(ctx,-60,8,22,22);
        Garaje.#rueda3D(ctx,55,8,22,22);
        ctx.fillStyle='#1a1a1a';
    ...
    static #carro3DPickup(ctx, color) {
        const cl = Garaje.#brillante(color,35), cd = Garaje.#brillante(color,-45), cs = Garaje.#brillante(color,-18);
        Garaje.#caraLateral(ctx,[[-90,0],[-90,-40],[-10,-40],[-10,0]],Garaje.#brillante(color,-30));
        Garaje.#caraLateral(ctx,[[-90,-40],[-10,-40],[-10,-46],[-90,-46]],cd);
        Garaje.#caraLateral(ctx,[[-12,0],[-12,-58],[-5,-72],[58,-72],[85,-55],[85,0]],color);
        Garaje.#caraLateral(ctx,[[-3,-72],[56,-72],[76,-58],[56,-55],[-3,-55]],cl);
        Garaje.#caraLateral(ctx,[[85,0],[95,-10],[95,-58],[85,-55]],cs);
        Garaje.#caraLateral(ctx,[[85,-55],[95,-58],[76,-62],[56,-55]],cd);
        ctx.fillStyle='rgba(100,200,255,0.48)';
        ...
        Garaje.#rueda3D(ctx,-58,4,24,16);
        Garaje.#rueda3D(ctx,54,4,24,16);
    }

    static #carro3DClasico(ctx, color) {
        const cl = Garaje.#brillante(color,35), cd = Garaje.#brillante(color,-45), cs = Garaje.#brillante(color,-18);
        Garaje.#caraLateral(ctx,[[-85,0],[-85,-40],[-70,-40],[65,-40],[82,-30],[82,0]],color);
        ctx.fillStyle=cl;
        ctx.beginPath();
        ctx.moveTo(-68,-40); ctx.bezierCurveTo(-68,-90,62,-90,62,-40);
        ctx.closePath(); ctx.fill();
        Garaje.#caraLateral(ctx,[[82,0],[92,-8],[92,-34],[82,-30]],cs);
        ctx.fillStyle=cd;
        ...
        ctx.closePath(); ctx.fill();
        ctx.strokeStyle=Garaje.#brillante(color,60); ctx.lineWidth=3;
        ...
        Garaje.#rueda3D(ctx,-52,4,22,14);
        Garaje.#rueda3D(ctx,50,4,22,14);
    }
}

window.Garaje = Garaje;
// Alias de compatibilidad
window.Garage = Garaje;
